...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; next?: string }> }) {
  ...
  const nextPath =
    params.next?.startsWith("/") && !params.next.startsWith("//")
      ? params.next
      : "/";
  const googleLoginUrl = `/api/auth/google?next=${encodeURIComponent(nextPath)}`;
  return <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,_rgba(20,184,166,0.25),_transparent_35rem)] p-6">
    <section className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-950/90 p-8 text-center shadow-2xl shadow-cyan-950/30">
      ...
      <a href={googleLoginUrl} className="mt-6 inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-primary px-5 py-3 font-semibold text-primary-foreground transition hover:bg-primary/90"><ShieldCheck className="h-5 w-5" />Continuar con Google</a>
