...
const navigation: { label: string; icon: typeof Boxes; href: string; permission: Permission }[] = [
  { label: "Activos", icon: Boxes, href: "/activos", permission: "activos:read" },
  { label: "Reportar falla", icon: ClipboardList, href: "/reportar-falla", permission: "fallas:create" },
  { label: "Órdenes de trabajo", icon: ClipboardList, href: "/ordenes-trabajo", permission: "ordenes:read" },
  { label: "Mantenimiento Preventivo", icon: ShieldCheck, href: "/mantenimiento-preventivo", permission: "preventivos:read" },
  { label: "Inventario", icon: Package, href: "/inventario", permission: "inventario:read" },
  { label: "Usuarios", icon: Users, href: "/usuarios", permission: "usuarios:read" },
  { label: "Historial", icon: History, href: "/historial", permission: "historial:read" },
  { label: "Indicadores", icon: Gauge, href: "/indicadores", permission: "indicadores:read" },
];
  ...
  return (
    <aside className="sticky top-0 z-20 flex border-white/10 bg-slate-950/90 backdrop-blur md:h-screen md:w-72 md:flex-col md:border-r">
      <div className="hidden items-center gap-3 border-b border-white/10 p-6 md:flex">
        <div className="rounded-2xl bg-primary p-2 text-primary-foreground"><Activity className="h-6 w-6" /></div>
        <div><p className="text-sm text-muted-foreground">GLA</p><h1 className="font-bold">Smart Maintenance</h1></div>
      </div>
      {user ? <div className="hidden border-b border-white/10 p-4 text-sm md:block">
